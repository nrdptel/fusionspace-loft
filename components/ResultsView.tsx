  ...
  return (
    <div className="space-y-8">
      ...
      {(doc.rocket.stages?.length ?? 1) === 1 && (
        <RocketpyCrossCheck
          key={`${doc.rocket.name}:${run.config.id}:${simIndex}:${ballastKg ?? 0}:${motorSwap?.designation ?? ""}:${geometry?.finSpan ?? 0}:${geometry?.finCount ?? 0}:${geometry?.finRootChord ?? 0}:${geometry?.finTipChord ?? 0}:${geometry?.noseLength ?? 0}:${geometry?.bodyLength ?? 0}:${geometry?.finish ?? ""}`}
          doc={doc}
          config={run.config}
          simIndex={simIndex}
          units={units}
      ...
      {(doc.rocket.stages?.length ?? 1) === 1 && swapOptions && swapOptions.length > 1 && (
        <MotorSweep
          key={`${doc.rocket.name}:${simIndex}:${ballastKg ?? 0}:${geometry?.finSpan ?? 0}:${geometry?.finCount ?? 0}:${geometry?.finRootChord ?? 0}:${geometry?.finTipChord ?? 0}:${geometry?.noseLength ?? 0}:${geometry?.bodyLength ?? 0}:${geometry?.finish ?? ""}`}
          doc={doc}
          simIndex={simIndex}
          units={units}
          options={swapOptions}
      ...
      {(doc.rocket.stages?.length ?? 1) === 1 && (
        <ParameterSweep
          key={`${doc.rocket.name}:${simIndex}:${ballastKg ?? 0}:${motorSwap?.designation ?? ""}:${geometry?.finSpan ?? 0}:${geometry?.finCount ?? 0}:${geometry?.finRootChord ?? 0}:${geometry?.finTipChord ?? 0}:${geometry?.noseLength ?? 0}:${geometry?.bodyLength ?? 0}:${geometry?.finish ?? ""}`}
          doc={doc}
          simIndex={simIndex}
          units={units}
          ballastKg={ballastKg}
