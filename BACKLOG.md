...
- **RESOLVED 2026-08-10, and the describer moved out of the component that owned it.** The breakdown
  gains a *Mass from* column on screen AND in its CSV — a copied table that says 984 g without saying
  who said so is the same wrong claim one screen further from the flyer. The words now live in
  `lib/mass-provenance.ts` rather than inside `components/GeometryInspector.tsx`, because THREE
  surfaces answer this question now and a describer local to one is a describer the other two cannot
  import. That is the same shape as this run's validation-table defect — a rule stated on the model,
  honoured by one surface, never asked by the next one built — so `lib/mass-provenance.test.ts`
  asserts no surface has grown its own copy of the sentences. Test files are exempt and say why: a
  case asserting a table renders "computed by the source tool" is a check on the vocabulary, not a
  second implementation. A row with no component id is the lumped mass of a stage-level override and
  shows the same dash the parts table uses — it stands for an assembly, so there is no one part to
  attribute.

- ~~**The mass & balance panel shows no provenance, while the parts table one panel away now does.**~~ *(resolved above.)* `components/MassBreakdown.tsx` renders each part's mass and its share of the dry total with no marker and no *Mass from* column, so the same 113 stated and 53 tool-carried figures read as bare numbers there. The parts table is the surface whose job is "did Loft read my rocket right?", which is why it went first; the breakdown is the surface a flyer reads to decide where the weight IS, and it should follow. `massSource` in `components/GeometryInspector.tsx` is already the one describer and is exported-shaped for the move.
- **Loft discards the figure it did not use, so it cannot show a disagreement RockSim shows.** A `.rkt` carries `<CalcMass>` and `<KnownMass>` on 67 of 67 parts across all four corpus designs, and the spreads are large — `FullScaleModelTH.rkt`'s tube coupler states 984.0 g against a computed 70.6 g, 13.9x. Loft now says WHICH kind of figure it flew and still cannot show the other one beside it, because the model carries one number. `COMPETITION.md` row 43 names this as the next thing on that row: it needs a second field, and it is the same shape as the cross-check panel's own argument that agreement builds confidence and disagreement is worth surfacing.
...
- **`lib/rasaero/adapt.ts:611` consumes a `.CDX1`'s `<Comments>` as the rocket NAME (first line only), where every other adapter now reads a design-level note into `Rocket.comment`.** Not destroyed and not wrong today — a RASAero file often has no other name — but it is the one importer that disagrees with the model about what that element is. Found by the pre-push review of the notes change.
- **`components/MassBreakdown.tsx:111` renders "% dry" at zero decimals while the CSV built from the same rows at `:60` uses one**, so every part under 0.5% of dry mass reads `0%` on screen and a real number in the copy — and on a high-power design that is a third of the rows, in the column whose whole job is *where does the dry mass come from*. `lib/display.ts:33`'s `fmtSmall` exists for exactly this and is used elsewhere on the same panel.
- **`components/MonteCarlo.tsx:231` advertises the REQUESTED sample count and `:588` reports the FLOWN one**, a screen apart, with nothing saying why they differ — `lib/sim/montecarlo.ts:312` drops samples for no propulsion, for failing `physicallyPossible`, and for any throw. `components/ParameterSweep.tsx:537` handles the identical situation by stating both. The "chance over ceiling" figure is computed over the flown subset, so the denominator matters.

- **`/docs/changelog` is generated from `CHANGELOG.md`, and the new 800-word / 2.5-per-1,000 prose budget now gates content no `.tsx` edit can chunk.** Today 591 words with 6 headings (2 + 4 `h4`) is comfortable, but the page grows one release at a time from `scripts/gen-version.mjs` and nothing caps a verbose release note. Counting the per-release `h4`s — done this run — buys a lot of room, since each release brings roughly four headings and ~500 words. Recheck if the floor ever trips: the fix is a cap in the generator, not a heading in a page nobody edits by hand.
- **The prose budget's headroom is 7%.** Measured 2026-08-09 on the export: worst runs are `/docs/methods` 747w and `/docs/limitations` 732w against the 800 cap. A paragraph added to either route's longest section trips `npm test`, with a message that names the run but not where to break it. Not wrong — the budget is meant to bite — but a session adding docs prose should expect it and run `DESIGN.md` §9's prose block before the gate.
- **`DESIGN.md` §9's prose block and `lib/docs-nav.test.ts` are two implementations of one rule.** They agree today (identical numbers on all five routes) and they read different page sets: the test also reads `out/docs.html`, the §9 block does not. Harmless only because the index is 339 words and both skip anything under 400. The §9 block is the readable statement and the test is the ratchet; if they ever disagree the test is right, and the block should be regenerated from it.
- **A fitting's "How many" does nothing on a shock cord, and the field is offered anyway.** `lib/sim/aero.ts` counts protuberance area for lugs and buttons only, and a shock cord is inside the airframe, so its count now reaches mass (fixed this run) and nothing else. That is arguably correct — three harnesses do weigh three times one — but the panel's caveat line explaining that the count is drag is deliberately hidden on shock cords, so the field arrives with no explanation of what it does. One line of help, or the count hidden where it means only mass.
- **`components/RocketDiagram.tsx:842` wears a third hue.** `fill-fuchsia-400 stroke-fuchsia-600` on mass markers, repeated at `:843` and on a legend dot at `:1124` — 7 occurrences (`grep -rn fuchsia components app`). §2 allows zinc plus indigo and says "no second accent". It is on the build surface a flyer looks at every session, and neither §9 nor `lib/design-system.test.ts`'s *lets no chrome wear a semantic ramp* can see a hue nobody thought to enumerate. From the design-system audit; reproduced by grep.
- **`components/LoftApp.tsx:2401` pins every design-editor label at `text-[11px]`.** `FIELDSET_LEGEND` and `FIELD_LABEL`, 13 call sites. §3 scopes `text-[11px]` to "axis ticks and diagram annotations only" and puts labels at `text-sm`, so this is every form label on the most-used surface a step below the smallest caption size. `components/ui.tsx:1245` already records the identical fix being made for `NumberField`, so the precedent is set and unapplied. From the design-system audit.
- **`ErrorState` has one call site and `EmptyState` has none outside the primitives.** Four error surfaces are hand-rolled and all four differ: `LoftApp.tsx:3838` (weather fetch, `text-xs text-red-600` — the pad-with-no-signal case), `ResultsView.tsx:747` (a bare `<span>`, so a failed log import never names the missing column), `PartPicker.tsx:676` (a raw `<p>`, with its loading state hand-rolled beside it), `RocketpyCrossCheck.tsx:313` (the most complete five-state implementation in the repo, and entirely unreusable). §5 requires all five states on every data surface. From the design-system audit.
- **`components/ParameterSweep.tsx` has neither an empty state nor an error state.** The only analysis surface scoring zero on both, against §5's "a surface with no empty state is not finished — it is the state a flyer sees first". From the design-system audit.
- **`components/ui.tsx:380`'s `Tabs` has zero call sites app-wide**, `WorkspaceNav.tsx:13` having swapped it for links when routes landed. This is exactly the case §5 deleted `Chip` for; leaving it exported reads as sanctioned vocabulary a future session will re-adopt.
- **`app/docs/validation/page.tsx:475` renders a raw `<table>` per RocketPy run** — Metric / RocketPy / Loft / Δ — against §5's "every table is `DataTable`". These are the most decision-grade numbers in the docs and they cannot be sorted or copied out. `DataTable` is a client component and these are static routes, so this is a decision rather than a conversion. Known and restated from run 7; the fan-out found it independently.
- **A pixel-geometry e2e spec failed once mid-shard and passed in isolation and on a full shard re-run.** `e2e/smoke.spec.ts:5039`, *a part can be dragged along the airframe*, on shard 2 of 2 with no fan-out running — which is the one detail that does not match the pattern `HANDOFF.md` recorded (CPU contention from a workflow). Two shard runs later it has not recurred. Filed rather than diagnosed: the next session that sees a lone mid-shard geometry failure should add it here rather than treating it as new.
- **`lib/model/edit.ts`'s `AIM_SLOTS` cannot express a target that applies to EVERY component kind**, and the next R12 member needs one — a mass override and a per-part comment are universal, while every slot today is keyed to a closed list of kinds. `components/LoftApp.tsx:2440` already carries a hand-written `nose:` escape hatch outside the registry (this entry said `:2415`, which is a docblock about label styling — corrected 2026-08-09), so there is a precedent for going around it; going around it a second time is the thing to avoid. Measured while scouting R12 increment 5.


- **The sibling's `SectionNav` renders its chips at `text-xs`, and `DESIGN.md` §3 says a label is `text-sm`.** Found by adopting the primitive into Loft, where §9's *has exactly 0 files where caption size outnumbers the body default* tripped at `1/0` — the component is nothing but chips. §3 scopes `text-xs` to "captions, units, footnotes" and puts "every label, value, body" at `text-sm`; a section's own name is the thing being navigated to, not a footnote about it. Loft's copy renders `text-sm px-3 py-1.5` (§4's padding for that size, reserving `px-2 py-1` for `text-xs` chips) and the sibling's still renders `text-xs px-2 py-1`. **The two implementations of one §5 entry now differ by a type size**, which is exactly what §10 exists to prevent — it needs a run that can gate both repos. Loft's is the one that meets the file.

- **The contents list marks no section on the first screen**, because `useCurrentSection` returns the last heading to have crossed the reading line and at scroll zero none has. Measured on `/docs/methods`: the marker appears from the second heading onward and the first screen shows a strip of chips with none current — which is the "a map with no 'you are here' is a list of place names" the primitive's own docblock complains about, on the one screen every reader sees. The fix is a fallback to the FIRST section when nothing has crossed the line (at the top of a document you are in its first section), and it belongs in `components/useCurrentSection.ts` — **which lives in both repos**, so it needs a run that can gate both. Loft's e2e asserts the contract the hook has today rather than the one it should have, and says so.

- **`/design` is 8.39 screens deep on a phone, against `DESIGN.md` §8's two.** Measured on the built export of `0639344` at 390x664: `document.scrollHeight` 5,574 px. Standing the airframe up this run added roughly 450 px of it (0.68 of a screen) and the rest predates the run — the drawing alone now starts **412 px down**, which is 1.5 screens of chrome and panel before the picture. §8's clause is about depth to the ANSWER and the answer (apogee, stability) is on `/flight`, so this is not a contract breach as written; it is the number the P-track needs before `/design` can be called finished, and nothing was measuring it. `e2e/depth.spec.ts` anchors `/design` on the drawing itself, which sits at the top of the panel and therefore cannot move.

- **`e2e/rocketpy-selfhosted.spec.ts:254` (`names the connection when the run fails with no signal`) failed once at session start and passed everywhere else.** Measured: shard 1/2 reported `122 passed, 1 failed` at `:276` (the assert after going back online and clicking *Try RocketPy again*); the same test alone passed in 0.9 s, the whole spec file passed 11/11 in 30 s, and a second full shard 1/2 was green at 123. A 7-agent fan-out was running against the same four cores at the time. **Same shape as the `depth.spec.ts:171` flake below — a single test mid-shard with everything around it green — and NOT the `EMFILE` shape**, which takes out a trailing block together with `ERR_CONNECTION_REFUSED`. A THIRD spec joined them later the same run: `touch.spec.ts:1031` (`counts the states a flyer at the pad cannot reach`) failed once in shard 2/2 and passed 19/19 when its own file was run immediately afterwards, then 123/123 on a re-shard. A FOURTH joined at the end of the run: `depth.spec.ts:103` (`no route is more than two screens deep`) failed once in shard 1/2 while this repo's own CI was running against PR #150, and passed 6/6 in isolation and 125/125 on a re-shard. Four different specs, every one of them measuring pixel geometry, every one while something else held the cores — so the pattern is contention, not any one spec. **Do not run a fan-out, another repo's CI, or anything else CPU-heavy alongside a sharded e2e run**, and re-run the shard before believing a single mid-shard failure.

- **`components/GeometryInspector.tsx:747` — one guard gates all six add verbs.** `onAddAfter && selectedId && kind === "bodytube"` wraps tube, fins, transition, mass object, coupler and centring ring, so nothing can be added while any other kind is picked. Repro: load any sample, pick the nose cone, watch every add control vanish. Feeds R12's verb-band member.
- **Moving a part is top-level-only by construction.** `moveSlots` (`lib/model/edit.ts:2613`) reads `rocket.stages[si].components` after finding the stage that directly contains the id, so a nested part has no slots at all. 419 of the corpus's 569 parts are nested. Repro: pick a mass object inside a body tube and look for a move affordance.
- **`ROADMAP.md` P8's "36 e2e call sites" is 40** — `smoke.spec.ts` 35, `touch.spec.ts` 4, `contrast.spec.ts` 1 — and one of them (`smoke.spec.ts:6858`) uses a different locator (`/Parts/` rather than `/Parts ·/`), so the grep that produced 36 could not see it. Corrected in the ledger rather than in the milestone, which is being rewritten anyway.
- **The "1,841 px journey" P8 cites for `/design` is not a `depth.spec.ts` measurement.** It is `components/MotorSweep.tsx:335`'s comment about scrolling to re-find a motor in a sixteen-option select. `e2e/depth.spec.ts:73` anchors `/design` on the drawing itself, which sits at the top of the panel, so content below it moves nothing that spec measures. P8's own correction 4 already said this; the stray figure is what keeps re-suggesting a depth budget that does not exist.
- **A subagent wrote three files into the working tree during this run** — `lib/ork/zz-probe.test.ts`, `lib/ork/zz-parentadapt.ts`, `lib/zz-cmp.test.ts` — despite an explicit read-only instruction in every prompt. They surfaced as lint errors (`no-require-imports`, `no-explicit-any`) and an unexplained test-file count of 61. Deleted. `MAINTAINING.md` already says subagents are read-only on the working tree; what it does not say is that they may ignore it, so **check `git status --porcelain --untracked-files=all` before every gate run**, not only before a commit.
- **`components/LoftApp.tsx:2016` truncates import warnings at six with no "+N more" and no count.** `Skipped unsupported component:` is pushed once per occurrence with no de-duplication (`lib/ork/adapt.ts:379`), so a file with six unknown tags silently drops every later warning — including the mass/CG caveat, which is pushed last. No corpus file exceeds six today, so this is latent rather than live.
- **An `<innerradius>` stated on an `<innertube>` is ignored entirely** (`lib/ork/adapt.ts`, the `innertube` parse arm reads `outerradius` and `thickness` only). OpenRocket writes `<thickness>` for these, so no OpenRocket-authored file hits it; a hand-edited or third-party file does, silently. Repro: an inner tube with a stated 39.7 mm bore inside a 76.2 mm outer radius imports at a 1.5 mm wall.
- **`e2e/touch.spec.ts:489`'s `boxes.filter(b => b.h < 44)` is vacuous today.** Every tap rect is `y={0} height={H}` — the whole diagram height — so the height assert cannot fail by construction. It is the assert P8 has to re-base on the MODEL axis; recorded here so the vacuity is not rediscovered as a P8 finding.
- **P8's own correction 2 undercounts `axis`'s consumers: five, not three.** The two pointer-to-value maps (`RocketDiagram.tsx:1253`, `:1345`) plus the `apply` memo's dependency list (`:1255`), on top of the cursor (`:1307`), `aria-orientation` (`:1311`) and the arrow glyph (`:1375`). And a sixth axis assumption is not gated by `axis` at all: the arrow-key handler at `:1326` hard-codes `ArrowLeft||ArrowDown` as decrement.
- **`RocketDiagram.tsx:185` is why P8 needs a height budget at all:** `const s = (W - 2 * padX) / o.length` derives pixels-per-metre from WIDTH alone, and `useMeasuredWidth` is the only measurement hook in the codebase. There is no height input anywhere in the file.
