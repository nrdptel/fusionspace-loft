...
describe("monteCarlo", () => {
  it("is deterministic in the seed — same seed reproduces the cloud exactly", async () => {
  ...
  it("a different seed gives a different cloud", async () => {
    const { rocket, opts } = await baseOpts();
    const a = monteCarlo(rocket, opts);
  ...
  it("flew (nearly) every requested sample — a resolvable design drops none", async () => {
    const { rocket, opts } = await baseOpts({ n: 80 });
    const r = monteCarlo(rocket, opts);
    expect(r.n).toBe(80);
    expect(r.samples.length).toBe(80);
  });

  it("zero dispersion collapses to a single deterministic outcome", async () => {
  ...
  it("more impulse spread widens the apogee band", async () => {
    const { rocket, opts } = await baseOpts();
  ...
  it("thrustScale raises apogee and total impulse monotonically", async () => {
    const doc = await load("demo-single-deploy.ork");
    const sim = doc.simulations[0];
  ...
  it("wind spread drives the landing scatter and recovery radius", async () => {
    const { rocket, opts } = await baseOpts();
  ...
  it("percentiles are ordered p5 ≤ p50 ≤ p95 within min/max", async () => {
    const { rocket, opts } = await baseOpts();
    const r = monteCarlo(rocket, opts);
