 ...
 *  anchor every other row is read against. On a design whose recovery opens before apogee that row is
 *  NOT the flight shown on the Flight card — on the bundled USLI airframe it reads 1,888 m against a
 *  flight of 342 m — and until this existed the two numbers simply disagreed on screen, one tab apart,
 *  with nothing joining them up. The footnote had always said the sweep is ballistic; what it never did
 *  was connect that to the specific number the flyer had just read.
 *
 *  Returns null when there is nothing worth saying, so the notice stays a signal rather than
 *  boilerplate. */
...
/** Every dimension the parameter sweep can vary. The union is derived from this list so a new axis
 *  is declared once — anything that needs to validate an axis at runtime (a remembered choice, say)
 *  reads the same array the type comes from. */
